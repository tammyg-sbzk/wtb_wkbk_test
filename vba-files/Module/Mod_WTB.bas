...
Dim BS_Sheet, PL_Sheet, WTB_Sheet, BSdol_Col, PLdol_Col, Find_Ctl, Find_Lookup, Tmp1_S, Tmp2_S As String
...
If Tmp1_I > 0 Then
    Worksheets(T_Sheet).Range("A1:A" & Tmp1_I).EntireRow.Delete
...
Dim WTB_Sheet, Tmp1_S As String
...
For I = WTB_End To WTB_Beg Step -1
    If Left(Worksheets(WTB_Sheet).Range("A" & I).Value, 4) = "<TOT" Then
        ...
        Worksheets(WTB_Sheet).Range("A" & I).EntireRow.Delete
    End If
Next I
...
Dim Row_Cogs, Row_GrossProfit, Tmp1_Row, Tmp2_Row, Tmp3_Row As String
...
ActiveSheet.Unprotect
For I = 1 To Col_Num
    FindColNumLtr WTB_Sheet, 1, Tmp1_I, Col_WTB(I), Col_Find(I)
Next I
FindRow WTB_Sheet, "A", SubTot_Beg, "<SUBTOT_BEG>"
FindRow WTB_Sheet, "A", SubTot_End, "<SUBTOT_END>"

For I = SubTot_End To SubTot_Beg Step -1
    'Debug.Print "Sub Tot>" & Worksheets(WTB_Sheet).Range(Col_WTB(2) & I).Value & "<"
    If Worksheets(WTB_Sheet).Range(Col_WTB(3) & I).Value = "SUBTOTAL" Then
        Sub_Beg = Worksheets(WTB_Sheet).Range(Col_WTB(4) & I).Value
        Sub_End = Worksheets(WTB_Sheet).Range(Col_WTB(6) & I).Value
        'Worksheets(WTB_Sheet).Range("A" & Sub_Beg & ":" & "A" & Sub_End).Value = "<" & UCase(Worksheets(WTB_Sheet).Range(Col_WTB(2) & I).Value) & ">"
        'Worksheets(WTB_Sheet).Range(Col_WTB(8) & Sub_Beg & ":" & Col_WTB(8) & Sub_End).Value = UCase(Worksheets(WTB_Sheet).Range(Col_WTB(2) & I).Value)
        Rows(Sub_End + 1).Insert Shift:=xlDown
        Worksheets(WTB_Sheet).Range("A" & Sub_End + 1).Value = "<TOT_BLANK>"
        Worksheets(WTB_Sheet).Range("A" & Sub_End + 1).RowHeight = 10
        Rows(Sub_End + 2).Insert Shift:=xlDown
        Worksheets(WTB_Sheet).Range("A" & Sub_End + 2).Value = "<TOT_SUB><" & UCase(Worksheets(WTB_Sheet).Range(Col_WTB(2) & I).Value) & ">"
        Worksheets(WTB_Sheet).Range(Col_WTB(1) & Sub_End + 2).Value = "TOTAL " & UCase(Worksheets(WTB_Sheet).Range(Col_WTB(2) & I).Value)
        ' Insert Totals
        Worksheets(WTB_Sheet).Range(Col_WTB(2) & Sub_End + 2).Formula = "=Subtotal(9," & Col_WTB(2) & Sub_Beg & ":" & Col_WTB(2) & (Sub_End + 1) & ")"
        Worksheets(WTB_Sheet).Range(Col_WTB(7) & Sub_End + 2).Formula = "=Subtotal(9," & Col_WTB(7) & Sub_Beg & ":" & Col_WTB(7) & (Sub_End + 1) & ")"
        With Worksheets(WTB_Sheet).Range(Col_WTB(1) & (Sub_End + 2) & ":" & Col_WTB(7) & (Sub_End + 2))
            .Font.Bold = True
            .Font.Size = 16
        End With
        Worksheets(WTB_Sheet).Range("A" & Sub_End + 2).RowHeight = 20
        Worksheets(WTB_Sheet).Range(Col_WTB(2) & (Sub_End + 2)).Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlContinuous
        Worksheets(WTB_Sheet).Range(Col_WTB(7) & (Sub_End + 2)).Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlContinuous
        Worksheets(WTB_Sheet).Range(Col_WTB(2) & (Sub_End + 1)).Borders(xlEdgeTop).LineStyle = XlLineStyle.xlContinuous
        Worksheets(WTB_Sheet).Range(Col_WTB(7) & (Sub_End + 1)).Borders(xlEdgeTop).LineStyle = XlLineStyle.xlContinuous
        
        Rows(Sub_End + 3).Insert Shift:=xlDown
...
If Row_Cogs > 0 Then
    For I = 1 To 2
    Worksheets(WTB_Sheet).Range("A" & (Row_Cogs + 2)).EntireRow.Insert Shift:=xlDown
    If I = 2 Then
        Worksheets(WTB_Sheet).Range("A" & (Row_Cogs + 2)).Value = Find_TotGrossProfit
        Worksheets(WTB_Sheet).Range(Col_WTB(1) & (Row_Cogs + 2)).Value = "GROSS PROFIT"
        Worksheets(WTB_Sheet).Range(Col_WTB(1) & (Row_Cogs + 2)).HorizontalAlignment = xlRight
        ...
        Str_1 = "=" & Col_WTB(2) & Tmp1_Row & "+" & Col_WTB(2) & Tmp2_Row
        Str_2 = "=" & Col_WTB(7) & Tmp1_Row & "+" & Col_WTB(7) & Tmp2_Row

        With Worksheets(WTB_Sheet).Range(Col_WTB(2) & Row_GrossProfit)
            .Formula = Str_1
            .Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlContinuous
        End With
        With Worksheets(WTB_Sheet).Range(Col_WTB(7) & Row_GrossProfit)
            .Formula = Str_2
            .Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlContinuous
        End With
    Else
...
If Row_Beg <> 0 And Row_End <> 0 Then
    ...
    Worksheets(WTB_Sheet).Range(Col_WTB(1) & (Row_End + 2)).Value = "TOTAL LIABILITIES AND EQUITY"
    Worksheets(WTB_Sheet).Range(Col_WTB(2) & Row_End + 2).Formula = "=Subtotal(9," & Col_WTB(2) & Row_Beg & ":" & Col_WTB(2) & Row_End & ")"
    Worksheets(WTB_Sheet).Range(Col_WTB(7) & Row_End + 2).Formula = "=Subtotal(9," & Col_WTB(7) & Row_Beg & ":" & Col_WTB(7) & Row_End & ")"
    With Worksheets(WTB_Sheet).Range(Col_WTB(1) & (Row_End + 2) & ":" & Col_WTB(7) & (Row_End + 2))
        .Font.Bold = True
    End With
    Worksheets(WTB_Sheet).Range(Col_WTB(1) & (Row_End + 2)).HorizontalAlignment = xlRight
    
    Worksheets(WTB_Sheet).Range(Col_WTB(2) & (Row_End + 2)).Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlDouble
    Worksheets(WTB_Sheet).Range(Col_WTB(7) & (Row_End + 2)).Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlDouble
...
Worksheets(WTB_Sheet).Range(Col_WTB(1) & Tmp1_I).Value = "NET (INCOME)/LOSS"
Worksheets(WTB_Sheet).Range(Col_WTB(2) & Tmp1_I).Formula = Str1
Worksheets(WTB_Sheet).Range(Col_WTB(7) & Tmp1_I).Formula = Str2
With Worksheets(WTB_Sheet).Range(Col_WTB(1) & Tmp1_I & ":" & Col_WTB(7) & Tmp1_I)
    .Font.Bold = True
End With
Worksheets(WTB_Sheet).Range(Col_WTB(4) & (Tmp1_I - 1)).Borders(xlEdgeTop).LineStyle = XlLineStyle.xlContinuous
Worksheets(WTB_Sheet).Range(Col_WTB(6) & (Tmp1_I - 1)).Borders(xlEdgeTop).LineStyle = XlLineStyle.xlContinuous
Worksheets(WTB_Sheet).Range(Col_WTB(2) & Tmp1_I).Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlDouble
Worksheets(WTB_Sheet).Range(Col_WTB(4) & Tmp1_I).Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlDouble
Worksheets(WTB_Sheet).Range(Col_WTB(6) & Tmp1_I).Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlDouble
Worksheets(WTB_Sheet).Range(Col_WTB(7) & Tmp1_I).Borders(xlEdgeBottom).LineStyle = XlLineStyle.xlDouble
...
Worksheets(WTB_Sheet).Range(Col_WTB(1) & I).Value = "Current Year Earnings"
Worksheets(WTB_Sheet).Range(Col_WTB(8) & I).Value = "EQUITY"
Worksheets(WTB_Sheet).Range(Col_WTB(2) & I).Formula = "=" & Col_WTB(2) & Tmp1_I
Worksheets(WTB_Sheet).Range(Col_WTB(7) & I).Formula = "=" & Col_WTB(7) & Tmp1_I
' Format 'Totals'
FindRow WTB_Sheet, "A", Tmp1_I, "<TOT_SUB><ASSETS>"
If Tmp1_I > 0 Then
    Worksheets(WTB_Sheet).Range(Col_WTB(1) & Tmp1_I).HorizontalAlignment = xlRight
    Worksheets(WTB_Sheet).Range(Col_WTB(2) & Tmp1_I).Borders(xlEdgeBottom).LineStyle = xlDouble
    Worksheets(WTB_Sheet).Range(Col_WTB(7) & Tmp1_I).Borders(xlEdgeBottom).LineStyle = xlDouble
End If
...
If Tmp1_I > 0 Then
    Worksheets(WTB_Sheet).Range(Col_WTB(4) & Tmp1_I).Borders(xlEdgeBottom).LineStyle = xlContinuous
    Worksheets(WTB_Sheet).Range(Col_WTB(6) & Tmp1_I).Borders(xlEdgeBottom).LineStyle = xlContinuous
End If
' Dr & Cr Totals
FindRow WTB_Sheet, "A", Row_Beg01, "<HDR>"
Row_Beg01 = Row_Beg01 + 1
FindRow WTB_Sheet, "A", Row_End01, Find_TotNetIncomeLoss
Str1 = "=Sum(" & Col_WTB(4) & Row_Beg01 & ":" & Col_WTB(4) & (Row_End01 - 2) & ")"
Worksheets(WTB_Sheet).Range(Col_WTB(4) & Row_End01).Formula = Str1
Str2 = "=Sum(" & Col_WTB(6) & Row_Beg01 & ":" & Col_WTB(6) & (Row_End01 - 2) & ")"
Worksheets(WTB_Sheet).Range(Col_WTB(6) & Row_End01).Formula = Str2
Worksheets(WTB_Sheet).Range("A" & Row_End01).RowHeight = 20
Worksheets(WTB_Sheet).Range(Col_WTB(1) & Row_End01 & ":" & Col_WTB(7) & Row_End01).Font.Size = 16
FindLastRow WTB_Sheet, Row_End02
Row_End02 = Row_End02 + 2
Worksheets(WTB_Sheet).Range("A" & (Row_End02)).Value = "<TOT_SUB><ADJUSTMENTS>"
Worksheets(WTB_Sheet).Range(Col_WTB(8) & (Row_End02)).Value = "AJE ID"
Worksheets(WTB_Sheet).Range(Col_WTB(1) & (Row_End02)).Value = "AJE DESCRIPTION"
With Worksheets(WTB_Sheet).Range(Col_WTB(8) & Row_End02 & ":" & Col_WTB(1) & (Row_End02))
    .Font.Bold = True
    .Font.Size = 16
    .RowHeight = 20
End With
...
With Worksheets(WTB_Sheet).Range(Col_WTB(8) & (Row_End02) & ":" & Tmp1_S & (Row_End02))
...
If Row_End01 > 0 Then
    ...
    Worksheets(WTB_Sheet).Range(Col_WTB(8) & (Row_End02 + 1)).PasteSpecial xlPasteValues
    Application.CutCopyMode = False
